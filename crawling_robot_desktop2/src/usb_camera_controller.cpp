...
UsbCameraController::UsbCameraController(QObject* parent) : QObject(parent) {
  captureTimer_ = new QTimer(this);
  captureTimer_->setTimerType(Qt::PreciseTimer);
  connect(captureTimer_, &QTimer::timeout, this, &UsbCameraController::captureFrame);
}

UsbCameraController::~UsbCameraController() = default;

void UsbCameraController::scanDevices() {
  QStringList devices;
  try {
...
void UsbCameraController::connectCamera(int deviceIndex, int fps,
...
void UsbCameraController::disconnectCamera() {
  captureTimer_->stop();
...
void UsbCameraController::shutdown() {
  disconnectCamera();
}

void UsbCameraController::captureFrame() {
