...
class SynchronizedDriveController final : public QObject {
 ...
 private slots:
  void controlTick();
  void onCanFrame(const crawling::CanFrame& frame);
  void onTransportChanged(bool connected, const QString& message);
  void onTransportError(const QString& message);
  void applyDelayedBrake();
 ...
 private:
  ...
  void sendStopPair(bool urgent = true);
  void sendZeroSpeedPair();
  void sendStatusQueryPair();
  void sendSpeedPair(double leftMps, double rightMps, bool urgent = false);
  double wheelSpeedToMotorDps(double wheelSpeedMps, int motorSign) const;
  MotorFeedback toPhysicalFeedback(const ServoFeedback& feedback, int motorSign) const;
  void publishTelemetry(const WheelTargets& output = {});
  void resetMotionState();

  DriveSettings settings_;
  SlcanTransport transport_;
  QTimer controlTimer_;
  QTimer brakeTimer_;
  QElapsedTimer clock_;
  qint64 lastTickMs_ = 0;
  qint64 armingStartedMs_ = 0;
  qint64 lastStopMs_ = -1000;
  qint64 lastFeedbackLogMs_ = -1000;
  qint64 lastStatusLogMs_ = -1000;
  qint64 lastUnknownFrameLogMs_ = -1000;
  qint64 lastDiagnosticQueryMs_ = -1000;
  qint64 lastCommandLogMs_ = -1000;
  qint64 lastTelemetryEmitMs_ = -1000;
  qint64 leftSpeedFeedbackMs_ = -1000;
  qint64 rightSpeedFeedbackMs_ = -1000;
  quint64 feedbackFrameCount_ = 0;
  InputCommand input_;
