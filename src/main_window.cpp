...
QString signedText(int sign) {
  return sign > 0 ? CRAWLING_TEXT("+1") : CRAWLING_TEXT("-1");
}
...
MainWindow::MainWindow(SynchronizedDriveController* controller, DeviceController* devices, LaserCorrectionController* correction, QWidget* parent)
  ...
  updateState(DriveState::Disconnected, CRAWLING_TEXT("\xE8\xAF\xB7""\xE8\xBF\x9E""\xE6\x8E\xA5"" SLCAN \xE9\x80\x82""\xE9\x85\x8D""\xE5\x99\xA8""\xE5\x90\x8E""\xE5\xBC\x80""\xE5\xA7\x8B""\xE3\x80\x82"""));
}
...
void MainWindow::updateManualCommand() {
  ...
  // A standalone left/right button is a forward arc command.  This keeps
  // both physical wheels moving in the same direction while DifferentialMixer
  // creates the requested speed difference. W/S still selects the travel
  // direction when held together with a turn button.
  double linear = (forward - reverse) * settings.maximumLinearSpeedMps * scale;
  if (forward == reverse && (left || right)) {
    linear = settings.maximumLinearSpeedMps * scale;
  }
  ...
  const double angular = (right - left) * settings.maximumAngularSpeedRadps * scale;
...
void MainWindow::updateTelemetry(const DriveTelemetry& telemetry) {
  ...
  setValueLabel(leftMotorValue_, CRAWLING_TEXT("转速 %1 度/s    电流 %2 A")
                                    .arg(telemetry.left.motorSpeedDps, 0, 'f', 1)
                                    .arg(telemetry.left.torqueCurrentA, 0, 'f', 2));
  setValueLabel(rightMotorValue_, CRAWLING_TEXT("转速 %1 度/s    电流 %2 A")
                                     .arg(telemetry.right.motorSpeedDps, 0, 'f', 1)
                                     .arg(telemetry.right.torqueCurrentA, 0, 'f', 2));
...
void MainWindow::buildInterface() {
  ...
  enableButton_ = makeTouchButton(CRAWLING_TEXT("底盘使能 / 松闸"), manualGroup,
                                  CRAWLING_TEXT("enableButton"));
  stopButton_ = makeTouchButton(CRAWLING_TEXT("停止底盘 / 抱闸"), manualGroup,
  ...
  auto* referenceButton = makeTouchButton(CRAWLING_TEXT("当前轮廓设为零点"), manualGroup);
  correctionActionGrid->addWidget(autoStartButton_, 0, 0);
  correctionActionGrid->addWidget(autoStopButton_, 0, 1);
  correctionActionGrid->addWidget(referenceButton, 1, 0, 1, 2);
  ...
  connect(referenceButton, &QPushButton::clicked, this, &MainWindow::setCorrectionReference);
  ...
  statusForm->addRow(CRAWLING_TEXT("左电机转速 / 电流"), leftMotorValue_);
  statusForm->addRow(CRAWLING_TEXT("右电机转速 / 电流"), rightMotorValue_);
  ...
  auto* adapterGroup = new QGroupBox(CRAWLING_TEXT("SLCAN \xE9\x80\x82""\xE9\x85\x8D""\xE5\x99\xA8"""), settingsPage);
  ...
  adapterForm->addRow(CRAWLING_TEXT("\xE4\xB8\xB2""\xE5\x8F\xA3"""), serialPortBox_);
  adapterForm->addRow(QString(), refreshButton);
  adapterForm->addRow(CRAWLING_TEXT("\xE9\x80\x82""\xE9\x85\x8D""\xE5\x99\xA8""\xE6\xB3\xA2""\xE7\x89\xB9""\xE7\x8E\x87"""), serialBaudBox_);
  adapterForm->addRow(CRAWLING_TEXT("CAN \xE6\xB3\xA2""\xE7\x89\xB9""\xE7\x8E\x87"""), canBitrateBox_);
  ...
  mappingForm->addRow(CRAWLING_TEXT("\xE5\xB7\xA6""\xE7\x94\xB5""\xE6\x9C\xBA"" CAN ID"), leftMotorIdBox_);
  mappingForm->addRow(CRAWLING_TEXT("\xE5\xB7\xA6""\xE7\x94\xB5""\xE6\x9C\xBA""\xE6\x96\xB9""\xE5\x90\x91"""), leftSignBox_);
  mappingForm->addRow(CRAWLING_TEXT("\xE5\x8F\xB3""\xE7\x94\xB5""\xE6\x9C\xBA"" CAN ID"), rightMotorIdBox_);
...
void MainWindow::bindController() {
  ...
  connect(devices_, &DeviceController::imuSampleChanged, correction_,
          &LaserCorrectionController::setImuSample, Qt::QueuedConnection);
  ...
  connect(devices_, &DeviceController::pointCloudProfileChanged, correction_,
          &LaserCorrectionController::processPointCloud, Qt::QueuedConnection);
...
void MainWindow::autoDetectHardware() {
  ...
  QMetaObject::invokeMethod(
      devices_, "autoDetectDevices", Qt::QueuedConnection,
      Q_ARG(QString, settings_.imuSerialPort),
      Q_ARG(QString, settings_.laserSerialNumber));
}

void MainWindow::applySensorDetection(const QString& imuPort, int imuBaudRate,
                                      const QString& laserSerialNumber) {
  if (!imuPort.isEmpty()) {
    setComboToText(imuPortBox_, imuPort);
    setComboToValue(imuBaudBox_, imuBaudRate);
  }
  if (!laserSerialNumber.isEmpty()) {
    laserSerialBox_->setText(laserSerialNumber);
  }
  ...
  QMetaObject::invokeMethod(controller_, "autoDetectCanDevices", Qt::QueuedConnection,
                            Q_ARG(QString, imuPort));
}

void MainWindow::applyCanDetection(const HardwareDetectionResult& result) {
  if (result.wheelDetected) {
    setComboToText(serialPortBox_, result.wheelCanPort);
    setComboToValue(serialBaudBox_, result.wheelSerialBaudRate);
    setComboToValue(canBitrateBox_, result.wheelCanBitrate);
    if (result.wheelMotorIds.size() >= 2) {
      leftMotorIdBox_->setValue(result.wheelMotorIds.at(0));
      rightMotorIdBox_->setValue(result.wheelMotorIds.at(1));
    }
  }
...
void MainWindow::stopAutoCorrection() { if (correction_) correction_->setEnabled(false); }
...
void MainWindow::releaseMotionButton(MotionKey key) {
  ...
  // removes the direction and sends a zero-speed 0xA2 command.  The previous
  ...
  // higher speed this looked like the brake engaging and releasing.
  ++motionPulseGeneration_[keyValue];
