...
bool SlcanTransport::writeFrames(const QVector<CanFrame>& frames, bool urgent) {
  ...
  // serial write so a left/right pair is parsed as two commands. Motion
  // commands are generated faster than the adapter can drain them, so stale
  // speed frames must not be allowed to build up behind the newest command.
  if (urgent) {
    transmitQueue_.clear();
    port_.clear(QSerialPort::Output);
  } else if (!encoded.isEmpty() && encoded.first().size() > 6 &&
             encoded.first().at(0) == 't' &&
             encoded.first().mid(5, 2).compare("A2", Qt::CaseInsensitive) == 0) {
    // 0xA2 is the cyclic speed command. Keep at most the latest complete
    // left/right pair; status and setup frames remain independently queued.
    transmitQueue_.clear();
  }
