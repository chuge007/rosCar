set "_CONNEXTDDS_DIR_BUILD=C:\connext\rti_connext_dds-7.3.0"
set "_CONNEXTDDS_ARCH=x64Win64VS2017"
  ...
  if "%CONNEXTDDS_IGNORE_BUILD%" == "" (
    ...
    if "%_CONNEXTDDS_DIR_ENV%" NEQ "" (
      if "%_CONNEXTDDS_DIR_ENV%" NEQ "%_CONNEXTDDS_DIR_BUILD%" (
        echo "[rti_connext_dds_cmake_module][warning] Using RTI Connext DDS installation configured during build-time (%_CONNEXTDDS_DIR_BUILD%). The installation specified via %_CONNEXTDDS_DIR_ENV_SRC% (%_CONNEXTDDS_DIR_ENV%) will be ignored. Use variable CONNEXTDDS_IGNORE_BUILD to override this behavior." 1>&2
      )
    )
...
if "%_CONNEXTDDS_DIR_EXPORT_ERROR%" NEQ "" (
  echo "[rti_connext_dds_cmake_module][warning] %_CONNEXTDDS_DIR_EXPORT_ERROR%. RTI Connext DDS will not be available at runtime, unless you already configured PATH manually." 1>&2
  goto cleanup_env
)
