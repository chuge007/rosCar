...
set "AMENT_ENVIRONMENT_HOOKS[libcurl_vendor]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[libcurl_vendor] "%AMENT_CURRENT_PREFIX%\share\libcurl_vendor\environment\ament_prefix_path.bat"
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[libcurl_vendor] "%AMENT_CURRENT_PREFIX%\share\libcurl_vendor\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[libcurl_vendor]:;=";"%") do (
    call:call_file %%f
  )
