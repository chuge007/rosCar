...
geometry_msgs__msg__WrenchStamped__create(void)
{
  ...
  geometry_msgs__msg__WrenchStamped__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    geometry_msgs__msg__WrenchStamped * data =
      (geometry_msgs__msg__WrenchStamped *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!geometry_msgs__msg__WrenchStamped__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          geometry_msgs__msg__WrenchStamped__fini(&output->data[i]);
        }
        return false;
      }
    }
