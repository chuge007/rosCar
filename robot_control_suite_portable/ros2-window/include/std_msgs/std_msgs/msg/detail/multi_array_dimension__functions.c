  ...
  std_msgs__msg__MultiArrayDimension__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    std_msgs__msg__MultiArrayDimension * data =
      (std_msgs__msg__MultiArrayDimension *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!std_msgs__msg__MultiArrayDimension__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          std_msgs__msg__MultiArrayDimension__fini(&output->data[i]);
        }
        return false;
      }
    }
