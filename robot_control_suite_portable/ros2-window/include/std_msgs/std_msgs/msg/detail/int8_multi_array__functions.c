  ...
  std_msgs__msg__Int8MultiArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    std_msgs__msg__Int8MultiArray * data =
      (std_msgs__msg__Int8MultiArray *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!std_msgs__msg__Int8MultiArray__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          std_msgs__msg__Int8MultiArray__fini(&output->data[i]);
        }
        return false;
      }
    }
