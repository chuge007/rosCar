  ...
  std_msgs__msg__UInt64MultiArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    std_msgs__msg__UInt64MultiArray * data =
      (std_msgs__msg__UInt64MultiArray *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!std_msgs__msg__UInt64MultiArray__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          std_msgs__msg__UInt64MultiArray__fini(&output->data[i]);
        }
        return false;
      }
    }
