  ...
  std_msgs__msg__Float32MultiArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    std_msgs__msg__Float32MultiArray * data =
      (std_msgs__msg__Float32MultiArray *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!std_msgs__msg__Float32MultiArray__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          std_msgs__msg__Float32MultiArray__fini(&output->data[i]);
        }
        return false;
      }
    }
