...
from crawling_robot_interfaces.msg import LaserCorrectionStatus, LaserProfile
...
class MonitorApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self._imu_panel_status = tk.StringVar(value="等待 /imu/data ...")
        self._imu_panel_age = tk.StringVar(value="数据延迟: --")
        self._profile_info = tk.StringVar(value="等待 /laser_profile/frame ...")
        ...
        self.linear_speed_var = tk.StringVar(value="0.005")
        self.angular_speed_var = tk.StringVar(value="0.030")
        self.linear_accel_var = tk.StringVar(value="0.500")
        self.angular_accel_var = tk.StringVar(value="2.000")
        self.wheel_speed_var = tk.StringVar(value="0.300")
        ...
        self._settings_loaded = self._load_settings()
    ...
    def attach_node(self, node: MonitorNode) -> None:
        ...
        if not self._settings_loaded:
            self.linear_speed_var.set(f"{node.max_linear_speed:.3f}")
            self.angular_speed_var.set(f"{node.max_angular_speed:.3f}")
            self.linear_accel_var.set(f"{node.max_linear_accel:.3f}")
            self.angular_accel_var.set(f"{node.max_angular_accel:.3f}")
            self.wheel_speed_var.set(f"{node.max_wheel_speed:.3f}")
            self.inner_wheel_ratio_var.set(f"{node.minimum_inner_wheel_ratio:.2f}")
    ...
    def _save_settings(self) -> None:
        values = {
            "linear_speed_m_s": self.linear_speed_var.get(),
            "angular_speed_rad_s": self.angular_speed_var.get(),
            "linear_accel_m_s2": self.linear_accel_var.get(),
            "angular_accel_rad_s2": self.angular_accel_var.get(),
            "max_wheel_speed_m_s": self.wheel_speed_var.get(),
            "minimum_inner_wheel_ratio": self.inner_wheel_ratio_var.get(),
    ...
    def _build_drive_controls(self, parent: ttk.Frame) -> None:
        ...
        fields = (
            ("线速度 (m/s)", self.linear_speed_var, 0.001, 0.300, 0.001),
            ("角速度 (rad/s)", self.angular_speed_var, 0.01, 2.00, 0.01),
            ("线加速度 (m/s²)", self.linear_accel_var, 0.001, 1.000, 0.001),
            ("角加速度 (rad/s²)", self.angular_accel_var, 0.01, 5.00, 0.01),
            ("车轮速度上限 (m/s)", self.wheel_speed_var, 0.001, 1.00, 0.001),
            ("转弯内/外轮比例", self.inner_wheel_ratio_var, 0.0, 1.0, 0.05),
        )
        for index, (label, variable, lower, upper, step) in enumerate(fields):
            ...
            ttk.Spinbox(
                ...
                width=8,
                format="%.3f",
            ).grid(row=row, column=column + 1, sticky="ew", padx=(6, 12), pady=2)
    ...
    def _build_sensor_tabs(self, parent: ttk.Frame) -> None:
        ...
        values = {}
        for key, label in (("roll", "横滚 Roll"), ("pitch", "俯仰 Pitch"), ("yaw", "航向 Yaw")):
    ...
    def update_imu(self, message: Imu) -> None:
        ...
        if self._imu_panel_values:
            ...
            self._imu_panel_status.set(f"在线 | frame_id: {message.header.frame_id or '--'}")
    ...
    def _apply_drive_limits(self) -> None:
        ...
        try:
            ...
            return
        if (not all(math.isfinite(value) and value > 0.0 for value in values[:4]) or
                not math.isfinite(values[4]) or values[4] <= 0.0 or
                not math.isfinite(values[5]) or not 0.0 <= values[5] <= 1.0):
            self.limits_var.set("速度、加速度和车轮上限必须大于零，内轮比例需在 0 到 1")
            return
        ...
        self._pressed.clear()
        self._publish_zero()
        self.limits_var.set("参数已应用")
        self._save_settings()

    def _apply_motor_mapping(self) -> None:
        if self._node is None:
    ...
    def _capture_localization(self) -> None:
        ...
        try:
            ...
            path = log_dir / f"localization_reference_{time.strftime('%Y%m%d_%H%M%S')}.json"
            path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
            self.status_var.set(f"定位感知已记录：{path.name}")
