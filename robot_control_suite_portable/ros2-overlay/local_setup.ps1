...
if ($env:COLCON_PYTHON_EXECUTABLE) {
  ...
  # use the Python executable known at configure time
  $_colcon_python_executable="D:\ros2\ros2-window\.pixi\envs\default\python.exe"
...
colcon_python_executable=(Join-Path (Split-Path $PSCommandPath -Parent) "..\runtime\python\python.exe")' 
  # if it doesn't exist try a fall back
  if (!(Test-Path "$_colcon_python_executable" -PathType Leaf)) {
    if (!(Get-Command "python" -ErrorAction SilentlyContinue)) {
      echo "error: unable to find python executable"
